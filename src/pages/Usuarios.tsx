...
export default function Usuarios() {
  ...
  const [created, setCreated] = useState<{ email: string; password: string | null; email_sent?: boolean; already_existed?: boolean } | null>(null)
  ...
  async function load() {
    ...
    const { data, error: fnError } = await supabase.functions.invoke('gisele-admin-users', {
      body: { action: 'list' },
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreated({ email: data.email, password: data.password, email_sent: data.email_sent, already_existed: data.already_existed })
    setForm({ nome: '', email: '' })
    load()
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Adicione funcionários para terem acesso ao passaporte de tratamento.</p>
      </div>

      <form onSubmit={handleCreate} className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
        <h2 className="text-sm font-bold text-gray-700">+ Novo funcionário</h2>
        ...
        {created && (
          <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-sm text-green-800 space-y-1">
            {created.already_existed ? (
              <p className="font-semibold">✓ <strong>{created.email}</strong> já tinha login e agora tem acesso a este sistema também.</p>
            ) : created.email_sent ? (
              <>
